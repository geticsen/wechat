	 ...
	 private static String handleTextMessage(Map<String, String> map) {
		 ...
		 String content = map.get("MsgType");
		 	...
		 	if(content.equals("text")){
				...
				// b.execute(sql);
				ResultSet rs = b.executeQuery("select  *from  msg");
				ArrayList<msg> list = new ArrayList();
				try {
					while (rs.next()) {
						int id = rs.getInt("id");
						String message = rs.getString("message");
						msg t = new msg(id,message);
						list.add(t);
					}
				...
				msg t = list.get(randnumber);
		 		
		 		String msgText = map.get("Content")+t.message+"\n";
...
/*		 	case "图文":
 ...
 class msg{
		int id;
		String message;
		public msg(int id,String msString){
			this.id=id;
			this.message=msString;
		}
